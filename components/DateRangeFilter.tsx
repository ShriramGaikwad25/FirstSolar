...
/** Single-box trigger that opens a two-month range calendar; reports the committed range as ISO (yyyy-mm-dd) strings. */
export function DateRangeFilter({
  ...
  const closePanel = useCallback(() => {
    ...
    if (current?.length === 1) {
      ...
      const committed = commitRange(current, (from, to) => onChange(from, to));
  ...
  const handleRangeChange = (dates: DateObject[] | null) => {
    ...
    if (dates.length >= 2) {
      const committed = commitRange(dates, (from, to) => onChange(from, to));
      committedRef.current = { ...committed };
    }
  };
  ...
  const displayTitle = useMemo(() => {
    const range = pickerRange?.length
      ? formatRangeLabel(
    ...
    return `Select date range for ${label}`;
  }, [pickerRange, dateFrom, dateTo, label]);
  ...
  const calendarPanel =
    ...
    createPortal(
      <div
        ...
        role="dialog"
        aria-label={`Choose date range for ${label}`}
      >
  ...
  return (
    <div
      ref={anchorRef}
      className={`relative flex h-[38px] w-full min-w-0 overflow-hidden rounded-md border border-gray-300 bg-white focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 ${className ?? ""}`}
    >
      <button
        type="button"
        onClick={() => (open ? closePanel() : openPanel())}
        className="relative flex min-w-0 flex-1 items-center overflow-hidden bg-white py-2 pl-8 pr-7 text-left text-sm"
        aria-label={`Select date range for ${label}`}
        aria-expanded={open}
        title={displayTitle}
      >
        <CalendarIcon
